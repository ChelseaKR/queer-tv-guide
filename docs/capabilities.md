...
DOC-21). Automated evidence shows behavior under test fixtures and static
...
| Licensed, attributed data | Shipped | Every source was read, quoted and snapshotted before use; the pipeline re-reads LezWatch.TV's terms on every run and stops if the reuse grant is gone; the snapshot carries the CC BY-SA 4.0 notice and both sources' credits; every show and character screen links its LezWatch.TV page, and every TVmaze schedule shown carries TVmaze's credit and license link (audited 2026-09-17, DECISIONS 0013). | [licenses and attribution audit](LICENSES-AND-ATTRIBUTION.md), [terms gate tests](../pipeline/tests/test_terms.py), [terms snapshot checksums](../pipeline/tests/test_terms_snapshots.py), [snapshot attribution tests](../pipeline/tests/test_attribution.py), [app attribution tests](../ios/GuideCore/Tests/GuideCoreTests/AttributionTests.swift) | Not a legal opinion. LezWatch.TV's grant is a terms-of-use page that can change, not a formal license. |
