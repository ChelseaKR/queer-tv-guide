...
| Licensed, attributed data | Shipped | Every source was read, quoted and snapshotted before use; the pipeline re-reads LezWatch.TV's terms on every run and stops if the reuse grant is gone; the snapshot carries the CC BY-SA 4.0 notice and both sources' credits. | [licences](LICENSES-AND-ATTRIBUTION.md), [terms gate tests](../pipeline/tests/test_terms.py), [terms snapshot checksums](../pipeline/tests/test_terms_snapshots.py) | Not a legal opinion. LezWatch.TV's grant is a terms-of-use page that can change, not a formal licence. |
