...
/// A favourites backup is a file the user holds (DATA-GOVERNANCE-STANDARD
...
///     }
///
/// It holds only what `FavouritesStore` holds: the kind, the snapshot id and
...
public enum FavouritesBackup {
    public static let format = "favourites-backup"
    public static let version = 1

    /// The largest file import will read. Favouriting every show and
    ...
    public enum ImportError: Error, Equatable, LocalizedError {
        ...
        public var errorDescription: String? {
            switch self {
            case .tooLarge:
                return "This file is too large to be a favourites backup."
            case .notABackup:
                return "This file isn't a favourites backup from this app."
            case .unsupportedVersion:
                return "This favourites backup was made by a newer version of the app. Update the app, then import it again."
            case .tooManyEntries(let n):
                return "This file lists \(n) favourites, more than a backup from this app can hold."
            }
        }
    }
    ...
    public struct Parsed: Equatable, Sendable {
        ...
        public let entries: [FavouritesStore.Entry]
        /// Well-formed entries whose show or character is not in the current
        /// snapshot. Skipped.
        public let notInSnapshot: Int
        /// Entries that are not a favourite this app can read (a wrong kind,
    ...
    /// the same favourites always export to the same bytes.
    public static func export(_ entries: [FavouritesStore.Entry]) throws -> Data {
    ...
    public static func parse(_ data: Data, now: Date, isKnown: (FavouritesStore.Kind, String) -> Bool) throws -> Parsed {
        ...
        var entries: [FavouritesStore.Entry] = []
        var seen: Set<String> = []
        var notInSnapshot = 0
        var unreadable = 0
        for item in items {
    ...
    /// One file entry, or nil if it is not a favourite this app can read.
    static func entry(from item: Any, now: Date) -> FavouritesStore.Entry? {
        guard let object = item as? [String: Any],
              let kindName = object["kind"] as? String,
              let kind = FavouritesStore.Kind(rawValue: kindName),
        ...
        return FavouritesStore.Entry(kind: kind, id: id, addedAt: added)
    }

    /// The snapshot contract's id shapes (`schema/snapshot.v1.json`):
    /// `lwtv:show:<digits>` and `lwtv:character:<digits>`, and the kind must
    /// match the id.
    public static func isWellFormed(_ id: String, kind: FavouritesStore.Kind) -> Bool {
