...
def test_field_presence_counts_both_sides():
    ...
    characters = [
        {"death": {"death_known": True}, "gender": {"slug": "x"}, "sexuality": None, "romantic": None, "actors": []},
        {"death": {"death_known": False}, "gender": None, "sexuality": None, "romantic": None, "actors": []},
    ]
...
def test_tvmaze_coverage_categorizes_every_miss_reason():
    shows = [
        {"schedule": {"schedule_known": True, "join": {"method": "lwtv_tvmaze_id"}}, "_join_ignored_by_source": False},
        {"schedule": {"schedule_known": False, "join": {"method": "none"}}, "_join_ignored_by_source": False},  # no_key
        {"schedule": {"schedule_known": False, "join": {"method": "none"}}, "_join_ignored_by_source": True},  # ignored
        {"schedule": {"schedule_known": False, "join": {"method": "lwtv_tvmaze_id"}}, "_join_ignored_by_source": False},  # not_found
    ]
...
def test_summary_lines_report_two_numbers_for_every_source():
    coverage_doc = {
        ...
        "tvmaze": {
            "shows_total": 10, "with_join_key": 8, "joined": 6, "join_rate": 0.6,
